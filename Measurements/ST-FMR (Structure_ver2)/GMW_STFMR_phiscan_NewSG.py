...
from GUIBaseClass_phiscan_GMW import GUIBase
from GUIBaseClass_phiscan_GMW import animate_plot
import time

sys.path.append(os.getcwd())  # add path to import dictionary
defaults = importlib.import_module('FieldControls_NewSG_GMW',
...
def fix_param1(index, output, delay, resources, kwargs):
    #output is phi, and it will only affect the measure_y in MeasurementClass_GMW.py
    ...
    resources['sig_gen_8257'].power = 20
...
def measure_y(output, magnet_recipe, delay, resources, fix1_output, fix2_output, kwargs):
    #setattr(resources['dsp_lockin'], kwargs['Hx Dac'],
    #        output / float(kwargs['Hx Conversion']))
    resources['gmw'].writetask.write(magnet_recipe)
    #time.sleep(delay)
    time.sleep(0.3)
    # time.sleep(0.6)
...
def main():
    ...
    loop_commands = {
        ...
        'module_name': 'GMW_STFMR_phiscan_NewSG',
        'fix1_start': 'phi start',
        'fix1_stop': 'phi stop',
        'fix1_step': 'phi step',
