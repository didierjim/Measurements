...
from MeasurementClass_phiscan_GMW import Measurement
...
class GUIBase(tk.Tk):  # Base GUI class with all features universal to code environment
    ...
    def __init__(self, _graph, _machines, order_dict, *args):
        ...
        self.settings = args
        self.graph = _graph
        self.graph.setdefault('graph_name', 'graph_name')
        self.machines = _machines
        self.results = {'x_data': mp.Array('d', 2000),
                        'y_data': mp.Array('d', 2000),
                        # for cases where Gaussmeter data is recorded
                        'x2_data': mp.Array('d', 2000),
        ...
        if os.path.isdir(os.path.expanduser('~\\Desktop\\Chia-Chin\\new-folder')):
            pass
        else:
            os.mkdir(os.path.expanduser('~\\Desktop\\Chia-Chin\\new-folder'))

        # save parameters
        self.directory = os.path.expanduser('~\\Desktop\\Chia-Chin\\new-folder')
    ...
    def output_method(self):
        ...
        try:
            lockin = instruments.nalrecovery.DSP7265(
...
def animate_plot(i, ax, _graph, _results, progress_bar, time_var, animate_x2=False):  # animation function
    # animation title doesn't show fix value
    ax.clear()
    ax.grid(True)
    ax.set_title(_results['graph_title'].value.decode())
...
def main():  # test version of the GUI_base and animation
    ...
    loop_commands = {
        '_1': 'testf1',  # name of fixed parameter one function
        'fixed_func_2': 'testf2',
