...
class GMW:
	...
	def __init__(self):
		...
		self.Field_X = self.readtask.ai_channels.add_ai_voltage_chan("VectorMagnet/ai0",max_val=9.5)
		self.Field_Y = self.readtask.ai_channels.add_ai_voltage_chan("VectorMagnet/ai1",max_val=9.5)
		self.Field_Z = self.readtask.ai_channels.add_ai_voltage_chan("VectorMagnet/ai2",max_val=9.5)
	...
	def loopshift_y(self, Hy, Hz, points):
		...
		if len(magV) < 4*points+1:
			...
			print('Now running Loop shift measurement at Hy %f Oe' %Hy)
			print('Loopshift max output current = %.2g A' %(10*np.max(magV)))
			return mag, magV
