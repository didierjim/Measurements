...
root = Tk()
root.title("Harmonic Hall Angular Scan")
...
def main():
    ...
    read_me = 'This program delivers a AC current and sensing current from the Lock-in. Hall resistance is measured per angle.'
...
def measureMethod(_number, _average, _signal, _frequency, _Hin, _sample, _angle, _resistance):
    ...
    sig=float(_signal)     # Lock-in Amp output signal voltage (Max 5)
    freq=float(_frequency) # Lock-in Amp signal frequency
    ...
    def event():
        ...
        for H in Hlist:
            ...
            amp = lockinAmp(func,sense,sig,freq,tc) #Initiate Lock-in
            gmw = GMW()
            recipe_list=[30,50,70,100,200,500,800,1000,1300,1500,1700,1900]
            if H in recipe_list:
                angmagV = np.loadtxt('C:/Users/doc/Desktop/GUI_Harmonic_Measurement/recipe/'+str(H)+'.csv',delimiter=',')
            ...
            ax.set_ylabel("Lock-In A-B (%s)" %(res_uni))
            graph_name = "Harmonic Angular Scan "+str(round(H,2))+"Oe "+str(round(sig,2))+"V"
            ax.set_title(graph_name)
            listbox_l.insert('end',"Now measuring with V = %f (V)" %(sig))
            ...
            for g in range(num+1):
                ...
                if state == "measure":
                    ...
                    if sen_uni=='mV':
                        data1 = amp.readX(average)/sig*res
                        print(amp.readX(average))
                        data2 = amp.readY(average)/sig*res
                    if sen_uni=='uV':
                        data1 = amp.readX(average)/sig*res*1000
                        data2 = amp.readY(average)/sig*res*1000
            ...
            file = open(str(directory)+"/"+str(_sample)+"_"+str(func)+"_"+str(H)+"Oe"+"_"+str(freq)+"Hz"+"_"+str(sig)+"V"+"_"+str(stamp), "w")
            ...
            file.write("Number"+" "+"Angle(degree)"+" "+"X("+res_uni+")"+" "+"Y("+res_uni+")"+" "+"R("+res_uni+") \n" )

            cnt=1
            #output all data
            for a in range(len(values_y1)):

                file.write(str(cnt)+" "+str(values_x[a])+" "+str(values_y1[a])+" "+str(values_y2[a])+" "+str(values_y3[a])+"\n")
                cnt +=1
...
def createWidgit():
    ...
    entry_resistance = ttk.Entry(frame_setting); entry_resistance.insert(0,"2.14")
    ...
    mode = ["1st","1st","2nd"]
    sensitivity = ["100mV","1V","500mV","200mV","100mV","50mV","20mV","10mV","5mV","2mV","1mV","500uV","200uV","100uV","50uV","20uV","10uV","5uV","2uV","1uV"]
    tc = ["100ms","5ms","10ms","20ms","50ms","100ms","200ms","500ms","1s"]
